...
pancreas.integrated <-RunHarmony(pancreas.integrated,'group',plot_convergenc=TRUE,assay.use="SCT")

pdf("05_ElbowPlot.pdf", width = 10)
ElbowPlot(object = pancreas.integrated)
dev.off()

pancreas.integrated <- FindNeighbors(pancreas.integrated, reduction = "harmony",dims = 1:opt$dims)
pancreas.integrated <- FindClusters(pancreas.integrated, resolution = opt$resolution)
pancreas.integrated <- RunUMAP(pancreas.integrated, reduction = "harmony",dims = 1:opt$dims)
...
plot1 <- DimPlot(object = pancreas.integrated, reduction = "umap", label = T,pt.size=1,group.by = c("ident", "group"),cols=mypalette)
plot3 <- DimPlot(object = pancreas.integrated, reduction = "umap", pt.size=1,group.by = "group",split.by = "group")
plot2 <- SpatialDimPlot(pancreas.integrated, stroke = 0, pt.size=8, label=T,label.size=2)
...
for(i in 1:length(file))
{
   ...
   plot1 <- SpatialDimPlot(BRAIN.list[[i]],stroke=0,pt.size=8,label=T,label.size=2)+scale_fill_manual(breaks=levels(Idents(pancreas.integrated)),values=mypalette)
   print(plot1)
   dev.off()
