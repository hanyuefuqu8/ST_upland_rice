...
genename[,'ID']<-make.unique(genename[,'ID'],sep="_")
...
d<-quantile(BRAIN.markers$p_val_adj,0.15)
BRAIN.marker<-BRAIN.markers[BRAIN.markers$p_val_adj<d,]
write.table(BRAIN.marker,file="markers.txt",sep="\t",quote=F)
marker.info <- left_join(BRAIN.marker,genename,by="gene")
write.csv(marker.info,file="markers.info.csv",quote=F,row.names=F)

top10 <- BRAIN.markers %>% group_by(cluster) %>% top_n(n = -10, wt = p_val_adj)
...
pdf ("marker_heatmap.pdf",width=9,height=20)
DoHeatmap(BRAIN, features = tops10$gene)
dev.off()
pdf("marker_dotplot.pdf",height=5,width=30)
...
ggsave2(paste0(name,".pdf"),sum,width=25,height=20)
